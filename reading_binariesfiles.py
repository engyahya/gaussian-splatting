...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        uid = intr.id
        #print (extr.name)
        # ptint a cam_RT/cam_RT_index.txt file with the following format:
        print (idx+1)
        # format the ind to 3 digits
        print ("{:03d}".format(idx+1))
        print ("cam_RT/cam_RT_"+str(idx+1)+".txt")

        R_mine = cam_RTs[idx][:3, :3]
        T_mine = cam_RTs[idx][:3, 3] 

        R = np.transpose(qvec2rotmat(extr.qvec))
        print (R_mine)
        print (R_mine.transpose())   
        print ("-" * 8)
        print (R)
        print ("*-" * 8)
